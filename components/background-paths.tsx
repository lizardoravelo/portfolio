...
function FloatingPaths({ position }: Readonly<{ position: number }>) {
  ...
  return (
    <div className='absolute inset-0 pointer-events-none'>
      <svg className='w-full h-full text-neutral-400 dark:text-neutral-600' viewBox='0 0 696 316' fill='none'>
        ...
        {paths.map((path) => (
          <motion.path
            ...
            transition={{
               // Safe use: randomness is only for visual animation duration, not for security
